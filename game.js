...
(() => {
  ...
  let snake, food, enemies, direction, queuedDirection, score, timer, state, effects, touchStart, bestScore = getBestScore();
  ...
  function occupied(position) { return snake.some(part => samePosition(part, position)) || enemies.some(enemy => samePosition(enemy.position, position)); }
  ...
  function resetGame() {
    snake = [{x:12,y:12},{x:11,y:12},{x:10,y:12}]; food = {x:17,y:12}; enemies = [];
    direction = "right"; queuedDirection = "right"; score = 0; effects = []; state = "ready";
    for (let i = 0; i < enemyCount; i += 1) enemies.push({ position:freeCell(), direction:Object.keys(directions)[Math.floor(Math.random() * 4)], phase:Math.random() });
  ...
  function moveEnemy(enemy) { const choices = Object.keys(directions).filter(candidate => candidate !== opposite[enemy.direction]); if (Math.random() < .35) enemy.direction = choices[Math.floor(Math.random() * choices.length)]; const vector = directions[enemy.direction]; const next = {x:enemy.position.x + vector.x, y:enemy.position.y + vector.y}; if (next.x < 0 || next.x >= gridSize || next.y < 0 || next.y >= gridSize) { enemy.direction = opposite[enemy.direction]; return; } enemy.position = next; }
  ...
  function step() {
    ...
    enemies.forEach(moveEnemy); effects = effects.map(effect => ({...effect,life:effect.life - .12})).filter(effect => effect.life > 0);
    const hitWall = head.x < 0 || head.x >= gridSize || head.y < 0 || head.y >= gridSize; const hitSelf = snake.some(part => samePosition(part, head)); const hitEnemy = enemies.some(enemy => samePosition(enemy.position, head));
    if (hitWall || hitSelf || hitEnemy) { if (hitEnemy) addEffect(head); endGame(); return; }
    snake.unshift(head); if (samePosition(head, food)) { score += 10; scoreElement.textContent = String(score); food = freeCell(); } else snake.pop();
    draw();
  }
  ...
  function draw() {
    ...
    drawCell(food, "#d6e94c", 3); enemies.forEach(enemy => drawCell(enemy.position, "#e9757d", 3)); snake.forEach((part, index) => drawCell(part, index === 0 ? "#f2efe7" : "#9bb32f", 2));
